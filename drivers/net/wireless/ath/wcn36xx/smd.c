...
static int wcn36xx_smd_process_ptt_msg_rsp(void *buf, size_t len, void**p_ptt_rsp_msg)
{
	...
	wcn36xx_dbg_dump(WCN36XX_DBG_HAL_DUMP, "HAL_PTT_MSG_RSP:", rsp->ptt_msg, rsp->header.len - sizeof(rsp->ptt_msg_resp_status));
	if (rsp->header.len > 0) {
...
int wcn36xx_smd_process_ptt_msg(struct wcn36xx *wcn,
			       struct ieee80211_vif *vif, void *ptt_msg, size_t len, void **ptt_rsp_msg)
{
	struct wcn36xx_hal_process_ptt_msg_req_msg *p_msg_body;
	int ret = 0;

	mutex_lock(&wcn->hal_mutex);
	p_msg_body = kmalloc(sizeof(struct wcn36xx_hal_process_ptt_msg_req_msg) + len, GFP_ATOMIC);
	...
	ret = wcn36xx_smd_process_ptt_msg_rsp(wcn->hal_buf, wcn->hal_rsp_len, ptt_rsp_msg);
	if (ret) {
